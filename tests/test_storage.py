...
def test_make_key_builds_partitioned_path_matching_spec_layout():
    key = make_key("bronze", "weblogs", {"ingest_date": "2024-06-07"}, "weblogs_chunk_001.parquet")
    assert key == "bronze/weblogs/ingest_date=2024-06-07/weblogs_chunk_001.parquet"
...
def test_make_key_supports_multiple_partitions():
    ...
    assert key == "quarantine/source=weblogs/etl_run_date=2024-06-07/etl_run_id=abc-123/anomalies.parquet"
...
def test_local_backend_write_then_read_round_trips(tmp_lake):
    ...
    key = "bronze/weblogs/ingest_date=2024-06-07/weblogs_chunk_001.parquet"

    path = tmp_lake.write_parquet(df, key)
    assert tmp_lake.exists(key)

    read_back = tmp_lake.read_parquet(key)
...
def test_local_backend_list_and_read_prefix_concatenates_chunks(tmp_lake):
    ...
    tmp_lake.write_parquet(df1, "bronze/weblogs/ingest_date=2024-06-07/weblogs_chunk_001.parquet")
    tmp_lake.write_parquet(df2, "bronze/weblogs/ingest_date=2024-06-07/weblogs_chunk_002.parquet")

    keys = tmp_lake.list("bronze/weblogs/ingest_date=2024-06-07")
    assert len(keys) == 2

    combined = tmp_lake.read_parquet_prefix("bronze/weblogs/ingest_date=2024-06-07")
    assert combined["a"].tolist() == [1, 2, 3, 4]


def test_local_backend_exists_false_for_missing_key(tmp_lake):
    assert tmp_lake.exists("bronze/weblogs/ingest_date=2024-01-01/missing.parquet") is False


def test_local_backend_read_parquet_prefix_empty_returns_empty_frame(tmp_lake):
    result = tmp_lake.read_parquet_prefix("silver/weblogs_clean/etl_run_date=2099-01-01")
    assert result.empty
...
# ---------------------------------------------------------------------------
def test_s3_backend_write_parquet_uses_upload_fileobj_with_transfer_config():
    mock_client = MagicMock()
    backend = S3Backend(bucket="my-bucket", client=mock_client, transfer_config="CONFIG_SENTINEL")

    df = pd.DataFrame({"a": [1, 2]})
    result = backend.write_parquet(df, "bronze/users/ingest_date=2024-06-07/users.parquet")

    mock_client.upload_fileobj.assert_called_once()
    args, kwargs = mock_client.upload_fileobj.call_args
    assert args[1] == "my-bucket"
    assert args[2] == "bronze/users/ingest_date=2024-06-07/users.parquet"
    assert kwargs["Config"] == "CONFIG_SENTINEL"
    assert result == "s3://my-bucket/bronze/users/ingest_date=2024-06-07/users.parquet"
...
def test_s3_backend_list_paginates_objects():
    ...
    mock_paginator.paginate.return_value = [
        {"Contents": [{"Key": "bronze/weblogs/a.parquet"}, {"Key": "bronze/weblogs/b.parquet"}]},
        {"Contents": [{"Key": "bronze/weblogs/c.parquet"}]},
    ]
    mock_client.get_paginator.return_value = mock_paginator
    backend = S3Backend(bucket="my-bucket", client=mock_client, transfer_config="CONFIG_SENTINEL")

    keys = backend.list("bronze/weblogs")
    assert keys == ["bronze/weblogs/a.parquet", "bronze/weblogs/b.parquet", "bronze/weblogs/c.parquet"]
